...
class linear_layer:
    ...
    def backward(self, X, grad):
        ...
        self.gradient['b'] = np.dot(np.ones((1, np.size(X, 0))), grad)
        #################################################################################################
...
class relu:
    ...
    def forward(self, X):
        ...
        forward_output = np.maximum(X, test)
        ################################################################################
    ...
    def backward(self, X, grad):
        ...
        test = np.zeros(X.shape)
        forward_output = np.maximum(X, test)
        u_deriv = np.where(forward_output==0,forward_output,1)
        backward_output = u_deriv*grad
        ####################################################################################################
