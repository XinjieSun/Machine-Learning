...
def distance_to_cluster_center(center,x,distance):
    dist=[]
    for i in range(len(x)):
        if len(distance)==0:
            ...
            res = [distance[i]]
        res.append(np.square(np.linalg.norm(x[i] - x[center])))
        dist.append(min(res))
    return dist
...
def get_k_means_plus_plus_center_indices(n, n_cluster, x, generator=np.random):
    ...
    while len(centers) < n_cluster:
        distance = distance_to_cluster_center(centers[-1], x, d)
        prob = [i/sum(distance) for i in distance]
...
class KMeans():
    ...
    def fit(self, x, centroid_func=get_lloyd_k_means):
        ...
        def distortion_value(x, y, centroids):
            distortion_measure = np.sum([np.sum((x[y == i] - centroids[i])**2) for i in range(self.n_cluster)])
            return distortion_measure
        y = np.zeros(N)
        iter_time = 0
        centroids = x[np.ix_(self.centers)]
        init_distortion = distortion_value(x, y, centroids)
        centroids_n = x[np.ix_(self.centers)]
        distance = np.zeros((self.n_cluster, N, D))
        while iter_time < self.max_iter:
            ...
            if abs(init_distortion-curr_distortion) <= self.e:
                break
...
class KMeansClassifier():
    ...
    def fit(self, x, y, centroid_func=get_lloyd_k_means):
        ...
        fitted_model = KMeans(self.n_cluster, self.max_iter, self.e, self.generator)
        centroids, membership, iter_time = fitted_model.fit(x, centroid_func)
        centroid_labels = []
        vote = {n: [] for n in range(self.n_cluster)}
        for i in range(self.n_cluster):
            for j in range(N):
                if membership[j] == i:
                    vote[i].append(y[j])
        for k in range(self.n_cluster):
            centroid_labels.append(np.argmax(np.bincount((np.asarray(vote[k])))))
    ...
    def predict(self, x):
        ...
        labels = self.centroid_labels[nearest]
...
def transform_image(image, code_vectors):
    ...
    X, Y, Z = image.shape
    x = image.reshape(X*Y, 3)
    distance = np.sum((x - np.expand_dims(code_vectors, axis=1)) ** 2, axis=2)
    element_index = np.argmin(distance, axis=0)
    new_im = code_vectors[np.ix_(element_index)].reshape(X, Y, Z)
    # DONOT CHANGE CODE BELOW THIS LINE
    return new_im
